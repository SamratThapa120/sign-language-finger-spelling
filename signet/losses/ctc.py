...
class CTCLoss:
    ...
    def sparse_calculation(self,y_true,y_pred):
        ...
        return tf.reduce_mean(tf.compat.v1.nn.ctc_loss_v2(sparse_y_true,y_pred,labellen,inplen,blank_index=self.blank_index,logits_time_major=False)) 
    ...
    def dense_calculation(self,y_true,y_pred):
        ...
        inplen = input_length * tf.ones(shape=(batch_len,), dtype=tf.int32)
        return tf.reduce_mean(tf.compat.v1.nn.ctc_loss_v2(y_true,y_pred,labellen,inplen,blank_index=self.blank_index,logits_time_major=False))   
